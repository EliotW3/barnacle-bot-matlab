...
function [gb, num] = groupBodies(bw)
    ...
    % second pass to update touching bodies
    for row = 2:size(group,1)-1
        for col = 2:size(group,2)-1
            if group(row,col) > 0
                % find the unique body IDs in the surrounding pixels

                for r = -1:1
                    for c = -1:1
                        if group(row + r, col + c) ~= group(row,col) && group(row+r,col+c) ~= 0
                            % a touching, unique ID has been found
                            current_id = group(row,col);
                            new_id = group(row +r, col + c);
                            group(group == current_id) = new_id;
                        end
                    end
                end
    ...
    % get the number of unique numbers within group, ignoring zero
    unique_ids = unique(group(group > 0)); 
    

    % return the grouped bodies and num bodies
    gb = group;
    num = length(unique_ids); 
